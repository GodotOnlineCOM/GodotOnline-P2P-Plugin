...
func _offer_created(type, data, id):
	if not rtc_mp.has_peer(id):
		return
	PrintHelper.debug("created %s" % type)
...
func _connected(id, use_mesh):
	...
	PrintHelper.debug("Connected %d, mesh: %s" % [id, use_mesh])
	if use_mesh:
...
func _disconnected():
	PrintHelper.debug("Disconnected: %d: %s" % [code, reason])
	if not sealed:
		stop() # Unexpected disconnect


func _peer_connected(id):
	PrintHelper.debug("Peer connected %d" % id)
	_create_peer(id)
...
func _offer_received(id, offer):
	PrintHelper.debug("Got offer: %d" % id)
	if rtc_mp.has_peer(id):
		rtc_mp.get_peer(id).connection.set_remote_description("offer", offer)


func _answer_received(id, answer):
	PrintHelper.debug("Got answer: %d" % id)
	if rtc_mp.has_peer(id):
